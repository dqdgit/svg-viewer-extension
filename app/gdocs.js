...
function GDocs(selector) {
  ...
  var BASE_URI_ = 'https://www.googleapis.com/drive/v3/';
  ...
  this.__defineGetter__('FILES_GET', function() {
    return BASE_URI_ + 'files';
  });
...
GDocs.prototype.getFileMetaData = function(fileId, callback) {
  var url = this.FILES_GET + "/" + fileId + "?fields=*";
  this.makeRequest('GET', url, function(response, e) {
    //Utl.log("GDocs.getFileMetadata - response=" + JSON.stringify(response) + ", e=" + JSON.stringify(e));
    callback(response, e);
  });
};
...
GDocs.prototype.loadFile = function(fileId, callback) {
  var url = this.FILES_GET + "/" + fileId + "?alt=media";
  this.makeRequest('GET', url, function(response, e) {
    callback(response, e);
  });
};
